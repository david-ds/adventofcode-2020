...
fn run_code(program: [700]?Instruction, swap_idx: usize) ?i64 {
...
fn run(input: [:0]u8) i64 {
    ...
    for (swaps) |swap_opt| {
        if (swap_opt) |swap_idx| {
            if (run_code(instructions, swap_idx)) |retval| {
                return retval;
            }
        } else {
            break;
