...
func (mp *Map) set(x int, y int, z int, w int, value bool) {
	if mp.minX > x {
		mp.minX = x
	}
	if mp.maxX < x {
	...
	mp.values[Coord{x, y, z, w}] = value
}

func (mp *Map) get(x int, y int, z int, w int) bool {
	value, exists := mp.values[Coord{x, y, z, w}]
	return exists && value
}
...
func (mp *Map) getOccupiedNeighborsCount(x int, y int, z int, w int) int {
	var count int
