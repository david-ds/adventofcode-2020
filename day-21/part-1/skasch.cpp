...
static std::unordered_map<std::string, std::string>
    kResolvedAllergenToIngredient;
static std::unordered_map<std::string, std::string>
    kResolvedIngredientToAllergen;
static std::unordered_map<std::string, int> kIngredientCounter;
...
std::string run(const std::string &input) {
  ...
  int res = 0;
  for (const auto &[ingredient, count] : kIngredientCounter) {
    if (auto it = kResolvedIngredientToAllergen.find(ingredient);
        it != kResolvedIngredientToAllergen.end()) {
      continue;
    }
    res += count;
  }
