...
class SubmissionPyx(SubmissionPy):
    def __init__(self, file):
        ...
        build_dir_relative = os.path.join(
            os.path.dirname(file), script_name.replace(".", "_") + "_build"
        )
        ...
        shutil.copyfile(
            resolve_path(file), resolve_path(build_dir, script_name_escaped)
        )

        try:
            cmd = "cd %s && python %s build_ext --inplace %s %s" % (
                build_dir,
                setup_path,
                script_name_escaped,
                build_dir,
            ...
            print(
                BColor.RED + "Could not compile cython %s" % script_name + BColor.ENDC
            )
            return

        rel_script_path = os.path.join(build_dir_relative, script_name_escaped)
        module_name = ".".join(
            os.path.normpath(os.path.splitext(rel_script_path)[0]).split(os.sep)
        )
