...
static constexpr int kCycles = 10'000'000;
static constexpr int kSize = 1'000'000;
...
std::string run(const std::string &input) {
  ...
  for (int value = input.size() + 1; value <= kSize; ++value) {
    kMap[value] = {value, nullptr};
    node->next = &kMap[value];
    node = node->next;
  }
  ...
  head = kMap[1].next;
  return std::to_string(head->data * head->next->data);
}
