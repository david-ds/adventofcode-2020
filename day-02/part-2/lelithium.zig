...
fn run(input: [:0]u8) i64 {
    ...
    while (all_lines_it.next()) |line| {
        ...
        var first_pos: i16 = std.fmt.parseInt(i16, line_it.next().?, 10) catch unreachable;
        var last_pos: i16 = std.fmt.parseInt(i16, line_it.next().?, 10) catch unreachable;
        var target_char: u8 = (line_it.next().?)[0];
        var occurences: i8 = 0;
        for (line_it.next().?) |cur_char, idx| {
            if ((idx == first_pos - 1 or idx == last_pos - 1) and target_char == cur_char) {
                occurences += 1;
            }
        }
        if (occurences == 1) {
