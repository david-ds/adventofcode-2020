...
let first_was_correct_letter = ref false
...
let iter c =
  if !jump > 0 then decr jump
    ...
    let new_state =
      match (!state, c) with
      | Start, _ ->
          first_was_correct_letter := false;
          Acc_low [ c ]
      ...
      | Count _, '\n' -> Start
      | Count i, c
        when Int.equal i !last_policy_low && Char.equal !last_policy_letter c ->
          first_was_correct_letter := true;
          Count (i + 1)
      | Count i, c when Int.equal i !last_policy_high ->
          if !first_was_correct_letter <> Char.equal !last_policy_letter c then
            incr count;
          SkipRest
      | Count i, _ -> Count (i + 1)
      | SkipRest, '\n' -> Start
